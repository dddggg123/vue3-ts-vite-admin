<template>
  <div class="menu-container">
    <el-menu
      :collapse="state.isSidebarNavCollapse" text-color="#a4a4a4" active-text-color="#fff"
      background-color="#191a23" :default-active="state.currentMenu" class="theme-bg" id="menu"
      :unique-opened="true">
      <template v-for="v in state.menuList" :key="v.name">
        <div v-if="v.children && v.children.length > 0 && !v.meta.subHide">
          ...
          <el-sub-menu :index="v.name" :key="v.name" class="theme-bg">
            ...
            <el-menu-item-group>
              ...
              <el-menu-item
                v-for="item in v.children" :key="item.name" :index="item.name"
                @click="navigateToRoute(item.name)" class="theme-bg">
                <el-icon>
        ...
        <div v-else>
          <!-- 内层 -->
          <el-menu-item
            v-if="!v.meta.hide" :key="v.name" :index="v.name" @click="navigateToRoute(v.name)"
            class="theme-bg">
            <el-icon>
              <component class="iconfont theme-color" :is="v.meta.icon" />
            </el-icon>
